...
function getQueryPrivacy(locale: Locale) {
	return qs.stringify({
		locale: locale,
		populate: {
			blocks: {
...
export async function generateMetadata({
	...
	if (locale === "de") {
		metaTitle = "Datenschutzrichtlinie - J42L"
		metaDescription = "Junior 42 Lausanne"
	} else if (locale === "fr") {
		metaTitle = "Politique de confidentialité - J42L"
		metaDescription = "Junior 42 Lausanne"
	} else {
		metaTitle = "Privacy policy - J42L"
		metaDescription = "Junior 42 Lausanne"
	}
...
export default async function Privacy({
	...
	try {
		const { lang: locale } = await params;
		const strapiData = await getStrapiData(path, getQueryPrivacy(locale));
