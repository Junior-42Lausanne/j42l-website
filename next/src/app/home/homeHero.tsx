...
export default function getHomeHero(strapiData: unknown): Hero {
	...
	if (heroArray && heroArray.length > 0) {
		return {
			...
			heroButton: {
				text: heroArray[0].button?.buttonText ?? heroDefault.heroButton.text,
				path: heroArray[0].button?.url ?? heroDefault.heroButton.path,
			},
			heroBackground: {
				src: heroArray[0].backgroundImage?.url ?? heroDefault.heroBackground.src,
				alt: heroArray[0].backgroundImage?.alternativeText ?? heroDefault.heroBackground.alt,
