// src/sections/portfolio/components/PortfolioServiceExplorer.tsx
    ...
    >
      <div className="mb-10 max-w-4xl">
        ...
        <p className={`${portfolioText.body} mt-5 max-w-2xl`}>
          Select a service and see the proof system update in the same space:
          capabilities, positioning and relevant project evidence.
        </p>
      </div>

      <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#1d1a15]">
        <div className="grid lg:min-h-[680px] lg:grid-cols-[0.38fr_0.62fr]">
          <ServiceIndex
          ...
          return (
            ...
            >
              ...
              <div className="self-center">
                ...
                <p
                  className={[
                    "mt-3 max-w-md text-sm leading-6 transition",
                    isSelected ? "text-white/68" : "text-white/42",
                  ].join(" ")}
                >
                  {service.description}
                </p>
              ...
              <div className="flex justify-end">
                <span
                  className={[
                    "mt-1 inline-flex h-9 w-9 items-center justify-center rounded-full border transition",
                    isSelected
                      ? "border-orange/35 bg-orange/10 text-orange"
                      : "border-white/10 bg-white/[0.02] text-white/24 group-hover:text-white/50",
                  ].join(" ")}
                >
...
function ServiceBrief({
  ...
  return (
    <article className="relative min-h-full overflow-hidden bg-[#211e18]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_16%_0%,rgba(244,152,25,0.12),transparent_34%)]" />
      ...
      <div className="relative flex min-h-full flex-col p-6 sm:p-8 lg:p-10 xl:p-12">
        ...
        <div className="mt-10 grid gap-10 xl:grid-cols-[minmax(0,1fr)_320px] xl:gap-12">
          <div>
            ...
            <h3 className="mt-4 max-w-2xl text-4xl font-semibold leading-[0.95] tracking-[-0.06em] text-white sm:text-5xl lg:text-6xl">
              {service.headline}
            </h3>
            ...
            <div className="mt-10 border-l border-orange/40 pl-5">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="h-4 w-4 text-orange" aria-hidden="true" />
          ...
          <aside className="border-t border-white/10 pt-8 xl:border-l xl:border-t-0 xl:pl-10 xl:pt-0">
            ...
            <div className="mt-8 space-y-6">
              {capabilities.slice(0, 3).map((capability) => (
                <div key={capability.id} className="border-t border-white/10 pt-5">
                  <p className="text-sm font-semibold text-white">
                    {capability.label}
                  </p>
          ...
          <div className="border-t border-white/10 pt-7">
            ...
            <div className="divide-y divide-white/10 border-y border-white/10">
              {relatedProjects.slice(0, 3).map((project, index) => (
                <div
                  key={project.slug}
                  className="grid gap-4 py-5 md:grid-cols-[0.12fr_0.36fr_1fr]"
                >
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-orange/80">
                    {String(index + 1).padStart(2, "0")}
                  </p>
